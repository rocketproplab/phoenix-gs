...
void dev_mode_logic() 
{
  /* ---------- 1. Update every valve as before ---------- */
  DebouncedInput *valve_list[] = {&gn2_lngFlow,
      ...
      for (DebouncedInput *item : valve_list)
      {
        ...
        if (item->currState != item->lastState)
        { printDevAndFueling(); }
      }

}

void printDevAndFueling(){
  ...
  const char *itemNames[8] = {
      "DEV MODE", // no ON/OFF printed here
      "GN2-LNG-F",
      "GN2-LOX-F",
      "LNG-F",
      "LOX-F",
  ...
  for (uint8_t i = 0; i < 8; ++i)
  {
    ...
    // i-1 maps to valve_list index (0-6)
      lcd.print(valve_list[i - 1]->currState ? "ON " : "OFF");
    /* "ON " vs "OFF" = 3 chars → keeps field length fixed (erases old data) */
  }
}
